...
#
#     twistd --logfile=/var/log/nespapi.log -n web --port tcp:8080 --wsgi nesp.api.api.app
#
...
logger = logging.getLogger("nesprestapi")
...
#     candidates = ['/opt/nesp/conf/nesprestapi.conf','nesprestapi.conf']
...
def makeService(cliconfig):
    ...
    observer = log.PythonLoggingObserver("nesprestapi")
    observer.start()
    # config = read_config(cliconfig) # JW - disabling this for now
    # setup python logging

    log_file = nesp.config.get("api", "log_file", default = "/var/log/nesprestapi.log")
    ...
    port = int(nesp.config.get("api", "port", default = 8080))
    logger.info("Using Port %d" % port)

    from nesp.api.api import app # TODO - make package naming less awkward
