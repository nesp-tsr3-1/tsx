<template>
  <div class="section">
    <div class="container is-widescreen program-view" v-if="program">
      <div class="columns">
        <div class="column is-8 is-offset-2">
          <user-nav></user-nav>

          <h2 class="title">{{ program.description }}</h2>

          <hr>

          <h4 class="title is-4">
            Program Details
            <router-link :to="{ name: 'ProgramEdit', params: { id: programId }}" tag="button" class="button is-small">Edit</router-link>
          </h4>

          <div class="columns">
            <div class="column">
              <div style="margin-bottom: 1em;">
                <div style="font-weight: bold;">Program Summary</div>
                {{ program.summary || 'N/A' }}
              </div>

              <div style="margin-bottom: 1em;">
                <div style="font-weight: bold;">Program Lead</div>
                {{ program.lead || 'N/A' }}
              </div>
            </div>
          </div>

          <h4 class="title is-4">Program Managers</h4>
          <program-managers :program-id="programId" :can-edit="canManageManagers"></program-managers>
          ...
          <div>
            ...
            <source-list
              ...
              :actions="['Remove']"
              @action="handleSourceAction"></source-list>
          </div>

         <div v-if="deletePermitted">
            <hr>

            <h4 class="title is-6">Delete Program</h4>
            <p class="content">
            ...
            <button class='button is-danger' :disabled="!enableDelete" v-on:click='deleteProgram'>Delete this program</button>
          </div>
    ...
    <div v-if="sourceToRemove" class="modal is-active">
      <div class="modal-background"></div>
      <div class="modal-card">
        <header class="modal-card-head">
          <p class="modal-card-title">Remove dataset</p>
        </header>
        <section class="modal-card-body">
          <h5 class="title is-5">{{sourceToRemove.description}}</h5>
        ...
        <footer class="modal-card-foot">
          <button class="button is-success" @click.once="removeSource">Remove dataset</button>
          <button class="button" @click="sourceToRemove = null">Cancel</button>
        </footer>
      </div>
    </div>
  </div>
