...
This filter uses a searchable drop down list of all SubIBRA regions with multiple selection allowed. If a state filter has also been specified, only regions in the selected state will be available for selection.
...
The filter uses a searchable drop down list with multiple selection allowed. For Program Managers, only programs that they manage are listed. For Administrators, all programs are listed, as well as a 'No program' option. The 'No program' option causes data with no associated monitoring program to be included.

For Program Managers, all of the programs they manage are automatically selected and they are prevented from deselecting all programs as this would result in an empty dataset. For Administrators, no programs are selected by default, which corresponds to no filtering of records based on program.
...
For individual datasets, a population trend that uses log-linear models only can be generated by clicking the 'Generate Populate Trend (log-linear)' button.
