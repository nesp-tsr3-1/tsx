...
A search bar above the table allows datasets to be searched by the following attributes:
...
As text is entered into the search bar, the table is immediately filtered to show only matching records.

By default, the search text is matched against _all_ of the attributes listed above, but if the search text is prefixed with an attribute name it will search for that attribute only. For example, entering "Custodian: joe" into the search bar will list only datasets with a custodian name that includes the text "joe". An auto-suggest drop down is used to show the available attribute prefixes so that users can easily discover them.

When the search text matches an attribute (besides description), the attribute name and value is displayed alongside the dataset description. Any text that matches the search text is higlighted in bold.

If the user enters a prefix into the search bar without any following text, then all datasets are listed and the attribute value(s) corresponding to that prefix are displayed. For example, if the user enters "Custodian:" into the search bar, all datasets are listed and all custodian names are displayed alongside the dataset description.
...
The dataset details page displays information related to an individual dataset and provides a variety of management functions.
...
=== No Data Sharing Agreement Warning

If the user is an administrator and the dataset does not have a data sharing agreement in place, a red warning banner is displayed at the top of the Dataset Details page with the text:
