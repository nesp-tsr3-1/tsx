...
. Unstandardised grid: surveys are processed exactly the same as standardised grid for the purposes of generating pseudo-absences, but are handled differently in the subsequent aggregation step.
...
NOTE: Running this command may take a few minutes which will depend on your computer and how much data you are processing.
