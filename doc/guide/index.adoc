...
There is a sample incidental sightings auxiliary input file at `sample-data/incidental_sightings.csv`, which can be imported by running:
----
...
// (TODO: for some reason we end up with an empty result, but when running python -m tsx.process -c all, we get a non-empty result). Need to figure out what is going on here.
