...
=== Installation using Docker (advanced)

These instructions assume Docker is installed (including Docker Compose) and that the reader is familiar with basic Docker usage.

Download and save the following file into a new folder: https://raw.githubusercontent.com/nesp-tsr3-1/tsx/master/containers/docker-compose.yml
...
After loading and starting the necessary containers, you will be presented with a bash prompt which you can use to follow along with the rest of this guide.
