...
The workflow was primarily designed to run on Linux and macOS, and requires several prerequisite software packages to be installed. Performing the installation directly onto a Windows is possible, but is complicated and not recommended for most users.

We also supply a Docker Compose configuration that is pre-configured to run the TSX workflow.
...
Download the TSX code from https://github.com/nesp-tsr3-1/tsx to your machine.

Using the command line/terminal, run the following command inside the folder containing the TSX code.

----
docker compose run --build --rm workflow_cli
----

The first time you run this command, Docker will need to download packages and build the necessary containers, which can take a few minutes. When the containers have been loaded, you will be presented with a command prompt which you can use to follow along with the rest of this guide. The command prompt looks like this:

----
TSX:/tsx#
----

The first time you run the TSX Workflow container, you will need to initialise the database by running:

----
TSX:/tsx# mysql -u root -proot tsx < db/sql/create.sql
TSX:/tsx# mysql -u root -proot tsx < db/sql/init.sql
TSX:/tsx# mysql -u root -proot tsx < sample-data/seed.sql
----


Please note the following:

 - The folder containing the TSX code will be shared with the Docker container, so you can access any files that you place in the folder from the TSX Workflow command prompt.
 ...
 - To completely reset the database and containers, run `docker compose --profile '*' down -v`. Then start the TSX workflow container again and run the commands above to initialise the database again.
...
----
uv run python -m tsx.import_taxa sample-data/TaxonList.xlsx
----
...
----
uv run python -m tsx.importer --type 1 -c sample-data/type_1_sample.csv
----
...
----
uv run python -m tsx.importer --type 2 -c sample-data/type_2_sample.csv
----
...
----
uv run python -m tsx.import_region sample-data/spatial/Regions-simplified.shp
----
...
----
uv run python -m tsx.import_processing_method sample-data/processing_method.csv
----

To aggregate the Type 1 data by month and year, run the following command:
----
uv run python -m tsx.process -c t1_aggregation
----
...
----
uv run python -m tsx.import_incidental sample-data/incidental_sightings.csv
----

There are sample range polygons at `sample-data/spatial/species-range/`, which can be imported by running:
----
uv run python -m tsx.import_range sample-data/spatial/species-range
----
...
----
uv run python -m tsx.process -c alpha_hull
----
...
----
uv run python -m tsx.process -c t2_aggregation
----
...
----
uv run python -m tsx.process -c spatial_rep
----
...
----
uv run python -m tsx.import_data_source sample-data/data_source.csv
----

The time series can then be filtered by running:
----
uv run python -m tsx.process -c filter_time_series
----
...
----
uv run python -m tsx.process export_lpi --filter
----
...
----
uv run python -m tsx.process export_lpi
----

or a version aggregated by month instead of year:
----
uv run python -m tsx.process export_lpi --monthly
----
...
----
uv run python -m tsx.process -c all
----
...
----
uv run python -m tsx.process -c all && uv run python -m tsx.process export_lpi --filter && (cd r; Rscript lpi.R)
----
...
// ----
// uv run python -m tsx.process export alpha
// 
// uv run python -m tsx.process export pa
// 
// uv run python -m tsx.process export ultrataxa
// 
// uv run python -m tsx.process export grid
// ----
...
---
uv run python -m tsx.importer --type 1 -c my-surveys.csv
---
