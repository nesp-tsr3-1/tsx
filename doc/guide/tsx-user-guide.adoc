...
The observation data must be provided to the workflow in a specific format and are classified as either Type 1 or Type 2 data. (TODO: Reference paper to describe Type 1 vs Type 2 data). The workflow performs much more complex processing on Type 2 data than Type 1 data, so if your data meets the Type 1 requirements then running the workflow will be quicker and easier.
...
A web interface can be used to import data and view the generated indices, diagnostics and processed data. This is not an essential element of the workflow and is covered in a separate section.
...
Open the virtual machine image using Virtual Box, and follow the prompts to import the image. On the "Appliance Settings" screen you can use the defaults, but for better performance adjust the CPU setting to match the number of CPU cores on your system.
...
You may also wish to install R Studio, which provides a graphical user interface and integrated development environment for running R scripts. This is not required to run the workflow. (https://www.rstudio.com/products/rstudio/download/#download)
...
Type 1 observation data may now be imported into the database. Some sample Type 1 data can be found in `sample-data\type_1_sample.csv`. Import this data, by running the following command:

----
python -m tsx.importer --type 1 -c sample-data\type_1_sample.csv
----
...
Type 2 data is imported in much the same way as Type 1 data. Sample Type 2 data can be found in `sample-data\type_2_sample.csv`. Import this data by running the following command:

----
python -m tsx.importer --type 2 -c sample-data\type_2_sample.csv
----
...
----
python -m tsx.import_processing_method
sample-data\processing_method.csv
----
...
This step of processing converts the species level observations to ultrataxon level observations using the range polygons auxiliary input that was imported in the previous step. In hybrid zones where a species maps to multiple subspecies we simply duplicate each species record for each potential subspecies.
...
The experimental design type that is used for a given taxon/source/unit/method combination is defined in the processing methods auxiliary input which was previously imported in the “Aggregate by year/month” step for Type 1 data.

To import the sample Type 2 sites auxiliary input (see Appendix 1 for specification), run:
----
python -m tsx.import_t2_site sample-data\spatial\t2_site.shp
----
...
#(TODO: Add some sample data that actually uses grid processing)
...
This step requires the range polygons auxiliary file to be imported first. If you skipped to this section from the Type 1 Data aggregation step, then you will need to import this now. A set of sample range polygons can be imported by running:
----
...
In order to calculate these filtering criteria two auxiliary data files must be imported: SubIBRA region polygons, and data source metadata. See the appendix for specifications of these files.

Sample data for these files can be imported by running:
----
...
(TODO: for some reason the filtering removes out all of the sample data – probably none of the sample data passes the criteria)
...
=== TODO: Generate LPI from wide table
...
include::appendix-file-formats.adoc[]